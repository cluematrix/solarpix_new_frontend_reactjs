...
const MSEBFieldList = () => {
  ...
  const fetchCustomField = () => {
    api
      ...
      .catch((err) => {
        console.error("Error fetching MSEB field:", err);
        setUserlist([]);
      });
  };
  ...
  const onSubmit = (values) => {
    ...
    if (editId) {
      // Update
      api
        .put(`/api/v1/admin/msebField/${editId}`, payload)
        .then(() => {
          successToast("MSEB field updated successfully");
          fetchCustomField();
          handleResetForm();
        })
        .catch((err) => {
          ...
          errorToast(
            err.response?.data?.message || "Failed to update MSEB field"
          );
        })
        .finally(() => {
          setLoadingBtn(false);
      ...
      // Add
      api
        .post("/api/v1/admin/msebField", payload)
        .then((res) => {
        ...
        .catch((err) => {
          console.error("Error adding MSEB field:", err);
          errorToast(
            err.response?.data?.message || "Failed to add MSEB field"
          );
        })
        .finally(() => {
          setLoadingBtn(false);
  ...
  const handleDeleteConfirm = () => {
    ...
    setLoadingBtn(true);
    api
      .delete(`/api/v1/admin/msebField/${deleteId}`)
      .then(() => {
        successToast("MSEB field deleted successfully");
        fetchCustomField();
        setShowDelete(false);
      })
      .catch((err) => {
        ...
        errorToast(
          err.response?.data?.message || "Failed to delete MSEB field"
        );
      })
      .finally(() => {
        setLoadingBtn(false);
      ...
      <Row className="mt-4">
        <Col sm="12">
          <Card>
            ...
            >
              <h5 className="card-title fw-lighter">MSEB Field</h5>
              {permissions.add && (
                <Button
