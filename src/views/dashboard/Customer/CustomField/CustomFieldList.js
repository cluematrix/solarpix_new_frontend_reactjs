...
const CustomFieldList = () => {
  ...
  const onSubmit = (values) => {
    ...
    if (editId) {
      // Update
      api
        .put(`/api/v1/admin/clientField/${editId}`, payload)
        .then(() => {
          successToast("Custom field updated successfully");
          fetchCustomField();
          handleResetForm();
        })
        .catch((err) => {
      ...
      // Add
      api
        .post("/api/v1/admin/clientField", payload)
        .then((res) => {
          successToast("Custom field added successfully");
          fetchCustomField();
          handleResetForm();
        })
        .catch((err) => {
  ...
  const handleDeleteConfirm = () => {
    ...
    setLoadingBtn(true);
    api
      .delete(`/api/v1/admin/clientField/${deleteId}`)
      .then(() => {
        successToast("Custom field deleted successfully");
        fetchCustomField();
        setShowDelete(false);
      })
      .catch((err) => {
      ...
      <Row className="mt-4">
        <Col sm="12">
          <Card>
            ...
            >
              <h5 className="card-title fw-lighter">Custom Field</h5>
              {permissions.add && (
                <Button
